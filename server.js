...
function submitAhhh() {
    ...
    // --- DEFINITIVE IOS BLOB FIX ---
    // Explicitly use the MIME type from the first recorded chunk.
    // This avoids Safari's "The string did not match the expected pattern" error.
    const blobOptions = { type: recordedChunks[0].type };
    const blob = new Blob(recordedChunks, blobOptions);
    // ---------------------------------
